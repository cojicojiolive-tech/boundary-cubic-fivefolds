...
#
# Title:
#   Adjacency detector for k = 3 (cubic fivefolds, Section 6)
#
# Purpose:
#   Given a collection of degree-3 exponent vectors (the monomials used for Case k = 3),
#   take every 5-subset together with the barycenter row η = (1,...,1) to form a 6×7
#   matrix. The 1-dimensional right kernel gives, up to scaling, the normal direction r ∈ Z^7
#   to the corresponding face; this coincides with a 1‑PS weight. We then print a
#   positionless “fingerprint” of r in the paper’s convention:
...
#
# Context:
#   This reproduces the micro-step behind the adjacency/wall-crossing computation for
#   Case k = 3 in Section 6; the eight monomials are exactly those appearing in §4.3.
#   The printed fingerprint matches the convention used in the paper; for k = 3 it equals
#   [[3, 2, 1, 1, -1, -2, -4]], agreeing with the adjacency pair {Φ3, Φ12}.  See Section 4.3
#   and Theorem 6.1.  (Shibata, *The Boundary of the Moduli Space of Stable Cubic Fivefolds*.)  # noqa
#
# How to run:
#   sage adjacency.sage
#
# Output:
#   Deduplicated fingerprints (paper/Mathematica convention). For this index set:
#       [[3, 2, 1, 1, -1, -2, -4]]
#
# Requirements:
...
#
# References:
#   Section 4.3 (Case k = 3) and Theorem 6.1 (adjacency) in the paper.  # see PDF
#   The script uses only linear algebra on exponent vectors.                                 
...
# ===== Exponent set for k = 3 (eight monomials of φ3) =====
#   x1*x3^2, x1*x3*x4, x1*x4^2, x2^2*x5, x0*x5^2, x1*x2*x6, x0*x3*x6, x0*x4*x6
#   Each vector lists exponents of (x0, x1, x2, x3, x4, x5, x6).
indexset = [
    vector(ZZ, [0, 1, 0, 2, 0, 0, 0]),  # x1 * x3^2
    vector(ZZ, [0, 1, 0, 1, 1, 0, 0]),  # x1 * x3 * x4
    vector(ZZ, [0, 1, 0, 0, 2, 0, 0]),  # x1 * x4^2
    vector(ZZ, [0, 0, 2, 0, 0, 1, 0]),  # x2^2 * x5
    vector(ZZ, [1, 0, 0, 0, 0, 2, 0]),  # x0 * x5^2
    vector(ZZ, [0, 1, 1, 0, 0, 0, 1]),  # x1 * x2 * x6
    vector(ZZ, [1, 0, 0, 1, 0, 0, 1]),  # x0 * x3 * x6
    vector(ZZ, [1, 0, 0, 0, 1, 0, 1]),  # x0 * x4 * x6
]
...
def main(print_debug=False):
    ...
    for subset in combinations(indexset, dim - 2):  # choose 5 of the 8
