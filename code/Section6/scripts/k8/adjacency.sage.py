...
_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_7 = Integer(7); _sage_const_3 = Integer(3)# adjacency_k8.sage
# -------------------------------------------
# Purpose:
...
def primitive_integer_vector(vv):
    ...
    """
    qv = [QQ(x) for x in vv]
    den_lcm = lcm([x.denominator() for x in qv]) if qv else _sage_const_1 
    ints = [int(x * den_lcm) for x in qv]
    # Make primitive by dividing by gcd of entries (ignoring zeros)
    nz = [abs(z) for z in ints if z != _sage_const_0 ]
    g = gcd(nz) if nz else _sage_const_1 
...
def normal_from_indexset(indexset):
    ...
    v0 = vector(QQ, pts[_sage_const_0 ])
    diff_rows = [vector(QQ, tuple(p)) - v0 for p in pts[_sage_const_1 :]]
    ones = vector(QQ, [_sage_const_1 ]*_sage_const_7 )  # enforces sum(r) = 0 (barycenter condition)
